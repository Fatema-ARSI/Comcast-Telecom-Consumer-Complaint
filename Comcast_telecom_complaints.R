...
#As we can observe that there are some complaints from different-different categories and we combined them into one, i.e.- others. So most of the complaints are related to Internet issue.
...
ggplot(as.data.frame(chart_data) ,mapping = aes(State,Count))+ geom_col(aes(fill = ComplaintStatus),width = 0.95)+ 
  theme(axis.text.x = element_text(angle = 90),
        axis.title.y = element_text(size = 15),
        axis.title.x = element_text(size = 15),
        title = element_text(size = 16,colour = "#0073C2FF"), 
        plot.title = element_text(hjust = 0.5))+ labs(title = "Ticket Status Stacked Bar Chart ",
...
chart_data%>% filter(ComplaintStatus == "Open")-> open_complaints 
open_complaints[open_complaints$Count == max(open_complaints$Count),c(1,3)]

#As we can observe that State Georgia has maximum number of unresolved tickets and these ticket count is 80.
...
#"With the help of above Chart of Total Resolved Vs Category Resolved we can conclude that the total resolved complaints are 77% in which 38% are received the internet and 39% are from the customer care calls.
#Also we can obserse that there are 23% complaints are still unresolved and in which 12% are received the internet and 11% are from the customer care calls.
